...
public class Ex19 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println("이름 두자 이상 입력하세요.");
		} catch (AgeValidException e) {
			System.out.println("나이는 0이상입니다.");
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			sc.close();
...
class User19 {
	...
	public void setName(String name) throws NameValidException { // Exception이 아버지니까 NameValidException 대신 Exception이 와도 됨 
		if(name.length() < 2) {
	...
	public void setAge(int age) throws AgeValidException { // Exception이 아버지니까 AgeValidException 대신 Exception이 와도 됨 
		if(age < 0) {
