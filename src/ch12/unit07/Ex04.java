...
public class Ex04 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			int n = 0;
			gogo:
			for(int col = 0; col < groupInwon; col++) {	// col : 조원
				for(int row = 0; row < groupCount; row++) { // row : 조
					group[row][col] = cc[n++]; // n 인원수
						if(n >= cc.length) {
							break gogo;	// **** 완전히 for문장을 나가기 위해서 왜?
						}
					}
				}
			...
			for(int i = 0 ; i <group.length; i++) {
				...
				for(int j = 0; j < group[i].length; j++) {
					if(group[i][j] != null) { // null은 출력되면 안되기 때문에?
