...
public class AddressUI {
	...
	protected void insert() {
		...
		try {
			...
			if(addr.findById(vo.getName(), vo.getTel()) != null) {  // 이 부분 꼭 안해도 됨
			...
			/*
			Calendar cal =  Calendar.getInstance();			// Calendar 다시 보기, 정리하기
			s = String.format("%tF(%tA)", cal, cal);
			*/
			LocalDate localDate = LocalDate.now();			// LocalDate 다시 보기, 정리하기
			DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy-MM-dd(E)"); // (E) : 요일
	...
	protected void update() {
		...
		try {
			...
			if(vo1 != null && vo != vo1) { // 이미 객체가 있는데, 주소가 다를경우 -> 다른객체를 있는 객체에 넣겠다는 의미
